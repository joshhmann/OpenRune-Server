...
data class BossFilter(
    ...
    override fun modifyEntry(entry: MetaRollable<Player, Item>) {
        ...
        println(
            "Modifying ${entry.identifier} by ${type.modifyBy} ($old -> $new (range[${entry.minChance} - ${entry.maxChance}])"
        )
...
/**
 * Mild example of an NPC-based pity drop system rather than player-based. This example never
 * guarantees a drop but just raises the chance of a drop happening, and then lowers it after it is
 * picked.
 */
val ExampleBossDrops =
    metaMultiChanceTable<Player, Item> {
