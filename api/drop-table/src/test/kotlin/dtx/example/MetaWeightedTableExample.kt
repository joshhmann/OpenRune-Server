...
enum class MetaModDirection(val operation: (Double, Double) -> Double, val opChar: Char) {
    Increase(Double::plus, '+'),
    Decrease(Double::minus, '-'),
}
...
data class OreFilter(
    ...
    override fun modifyEntry(entry: MetaRollable<Player, Item>) {
        ...
        println(
            "Modifying ${entry.identifier} by ${type.modifyBy} ($old -> $new (range[${entry.minimumWeight} - ${entry.maximumWeight}])"
        )
...
/**
 * Example of a """realistic""" mining node. The more you mine it the less of the other stuff you
 * get. But it also re-ups the things you don't get so not actually
 */
val ExampleOreNode =
    metaWeightedTable<Player, Item> {
        ...
        100 weight
            {
                ...
                rollable { Item("Stone", Random.nextInt(1, 4)) }
        ...
        33.33 weight
            {
                ...
                rollable { Item("Impure Ore", Random.nextInt(1, 3)) }
        ...
        10 weight
            {
                ...
                rollable { Item("Pure Ore", Random.nextInt(1, 2)) }
