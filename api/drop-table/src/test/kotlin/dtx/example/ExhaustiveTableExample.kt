...
val uniformExhaustiveTableExample =
    uniformExhaustiveTable<Player, Item> {
        ...
        1 rolls
            {
                onRollCompleted { player, args, encounter ->
                    println(
                        "You turn the corner and the head honcho is there, ready to beat you up."
                    )
                }
                onExhaust {
                    println(
                        "Well, there's usually only 1 head honcho so that's as hard as it'll get! Hopefully..."
                    )
                }
        ...
        3 rolls
            {
                onRollCompleted { player, args, encounter ->
                    println(
                        "A... skeleton riding a goblin? What? That can't be common around here..."
                    )
                }
                onExhaust {
                    println(
                        "You were right, that wasn't so common. All the... goblin jockeys... are dead."
                    )
                }
        ...
        6 rolls
            {
                ...
                onExhaust { println("That just might have been the last horde.") }
        ...
        10 rolls
            {
                ...
                onExhaust {
                    println(
                        "There can only be so many stray goblins, they usually stick together! That might be the last of them."
                    )
                }
                result(Item("Lone goblin"))
            }

        onExhaust { println("Yikes, out of encounters to roll! Reset the table!") }
    }

/** Very good for spicy raffles tbh, weights are based on how many rolls each item has */
val weightedExhaustiveTableExample =
    weightedExhaustiveTable<Player, Item> {
        name("Card raffle")
        ...
        onExhaust { println("Raffle is over, deck is empty!!") }
    }

fun main() {
    repeat(21) { uniformExhaustiveTableExample.roll(examplePlayer) }
