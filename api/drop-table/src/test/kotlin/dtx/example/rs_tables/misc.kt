...
inline fun Player.hasUnlockedScrollBoxes(): Boolean =
    checkQuestStatus(xmts_quest) is QuestStatus.Completed

inline fun Player.canCollectScroll(clueTier: ClueTier): Boolean =
    if (hasUnlockedScrollBoxes()) {
...
fun clueDrop(clueTier: ClueTier) =
    singleRollable<Player, Item> {
        ...
        onVeto { player ->
            val sneakingSuspicioun = buildString {
                append(
                    "You have a sneaking suspicion that you would have received a $clueTier scroll"
                )
...
fun championScroll(type: ChampionType) =
    singleRollable<Player, Item> {
        ...
        vetoRoll { target, args ->
            if (target.hasChampionScrollComplete(type) || target.posesses(scrollItem)) {
                ...
                target.sendMessage(
                    "You have a funny feeling that you would have recieved a Champion's scroll..."
                )
