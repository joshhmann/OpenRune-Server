...
fun <T, R, E : RSTable<T, R>> E.countRoll(
    rolls: Int,
    target: T,
    idSelector: (R) -> String,
    otherArgs: ArgMap = ArgMap.Empty,
    ...
    repeat(rolls) {
        when (val result = roll(target, otherArgs).flatten()) {
            ...
            is RollResult.ListOf ->
                (result.flatten() as RollResult.ListOf<R>).results.forEach { it.inc() }
        }
    }
}
...
fun oborRollComparison(player: Player, rolls: Int = 817_368) {
    ...
    println(
        fullResults.entries
            .filter { fr -> mainExpected.any { it.first == fr.key } }
            .sumOf { it.value.toDouble() / rollAmount }
    )
