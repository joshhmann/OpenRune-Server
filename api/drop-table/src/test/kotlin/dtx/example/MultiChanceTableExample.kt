...
val multiChanceTableExample =
    multiChanceTable<Player, Item> {
        name("Chicken")
                ...
                1 weight
                    {
                        onRollCompleted { player, args, result ->
                            ...
                            player.sendMessage(
                                "what the cluck, ${result.result.itemAmount} feathers?"
                            )
                        }
...
fun main() {
    repeat(1_000_000) { multiChanceTableExample.roll(examplePlayer) }
}
