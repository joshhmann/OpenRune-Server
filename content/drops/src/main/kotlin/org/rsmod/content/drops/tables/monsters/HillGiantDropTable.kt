...
@JvmField
public val hillGiantDropTable: RSDropTable<Player, DropRollItem> =
    RSDropTable(
        tableIdentifier = "Hill Giant Drops",
        npcs =
            npcs(
                "npc.avium_giant1",
                "npc.avium_giant2",
        ...
        preRoll = rsPlayerPrerollTable { 1 outOf 128 weight "obj.hillgiant_boss_key" count 1 },
        mainTable =
            rsPlayerWeightedTable(total = 134) {
                name("Hill Giant Drops")
                4 weight "obj.iron_dagger" count 1
                5 weight
                    "obj.iron_med_helm" count
                    1 condition
                    { player ->
                        // Drops Need Manual: Only dropped by giants on the Giants' Plateau.
                        true
                    }
                5 weight
                    "obj.iron_full_helm" count
                    1 condition
                    { player ->
                        // Drops Need Manual: Only dropped by giants not on the Giants' Plateau.
                        true
                    }
