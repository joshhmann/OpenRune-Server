...
@JvmField
public val zombieShayzienCryptsDropTable: RSDropTable<Player, DropRollItem> =
    RSDropTable(
        tableIdentifier = "Zombie (Shayzien Crypts) Drops",
        npcs = npcs("npc.ds2_zombie_magic", "npc.ds2_zombie_melee", "npc.ds2_zombie_ranged"),
        mainTable =
            rsPlayerWeightedTable(total = 120) {
                name("Zombie (Shayzien Crypts) Drops")
        ...
        tertiaries =
            rsPlayerTertiaryTable {
                1 outOf
                    ...
                    1 condition
                    { player ->
                        // Drops Need Manual: Zombie champion scrolls are only dropped if access to
                        // Champions' Guild is available, and the Zombie Champion hasn't been
