...
class SmeltOresNode : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        // Path to the furnace
        player.routeRequest = RouteRequestCoord(destination = nearestFurnace, clientRequest = false)
    ...
    private fun smeltInstant(player: Player): NodeStatus {
        val inv = player.inv
        val copperId =
            ServerCacheManager.getItem("obj.copper_ore".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
        val tinId =
            ServerCacheManager.getItem("obj.tin_ore".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
        val ironId =
            ServerCacheManager.getItem("obj.iron_ore".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
        val bronzeBarId =
            ServerCacheManager.getItem("obj.bronze_bar".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
        val ironBarId =
            ServerCacheManager.getItem("obj.iron_bar".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
...
class SmithBarsNode : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        player.routeRequest = RouteRequestCoord(destination = nearestAnvil, clientRequest = false)
        return NodeStatus.RUNNING
    }
    ...
    private fun smithInstant(player: Player): NodeStatus {
        ...
        val ironBarId =
            ServerCacheManager.getItem("obj.iron_bar".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
        val bronzeDaggerId =
            ServerCacheManager.getItem("obj.bronze_dagger".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
        val ironDaggerId =
            ServerCacheManager.getItem("obj.iron_dagger".asRSCM(RSCMType.OBJ))?.id
                ?: return NodeStatus.FAILURE
...
class ProductionTreeBuilder {
    companion object {
        fun build(): BehaviorNode {
            return SelectorNode(
                listOf(
                    SequenceNode(listOf(HasBarsNode(), SmithBarsNode())),
                    SequenceNode(listOf(HasOresNode(), SmeltOresNode())),
