...
object ChatResponseSystem {
    ...
    private fun loadConfig() {
        val file = File("/root/Runescape/open_rune/OpenRune-Server/lost-city-progressive-clone/engine/data/bot/chat_responses.json")
        if (file.exists()) {
            try {
            ...
            logger.warn { "[ChatResponseSystem] chat_responses.json not found at ${file.absolutePath}" }
        }
    }

    fun handleIncomingChat(sender: Player, bot: Player, message: String) {
        val cfg = org.rsmod.content.other.playerbotservice.BotConfig.config
        
        // Attempt LLM generation first
        queryLLM(sender, bot, message).thenAccept { llmResponse ->
            if (llmResponse != null && llmResponse.isNotBlank()) {
                bot.publicMessage = PublicMessage(
                    text = llmResponse,
                    colour = 0,
                    effect = 0,
                    clanType = null,
                    modIcon = bot.modLevel.clientCode,
                    autoTyper = false,
                    pattern = null
                )
            } else {
                // Fallback to pattern-matching rules
                respondWithPattern(sender, bot, message)
            }
        }
    }
    ...
    private fun queryLLM(sender: Player, bot: Player, message: String): CompletableFuture<String?> {
        ...
        val prompt = "You are a RuneScape player bot named ${bot.avatar.name}. " +
    ...
    private fun respondWithPattern(sender: Player, bot: Player, message: String) {
        ...
        for (intent in cfg.intents) {
            if (intent.patterns.any { cleanMsg.contains(it) }) {
                respondWith(sender, bot, intent.replies)
                return
            }
        }
        ...
        if (greetings != null && greetings.patterns.any { cleanMsg.contains(it) }) {
            respondWith(sender, bot, greetings.replies)
            return
        }
        ...
        if (help != null && help.patterns.any { cleanMsg.contains(it) }) {
            respondWith(sender, bot, help.replies)
            return
        }

        // 4. Optionally fallback with a small probability
        if (Random.nextInt(100) < 20) {
            respondWith(sender, bot, cfg.fallback)
        }
    }

    private fun respondWith(sender: Player, bot: Player, replies: List<Reply>) {
        ...
        val formattedText = formatReply(selectedReply.text, sender, bot)
    ...
    private fun formatReply(template: String, sender: Player, bot: Player): String {
        var result = template
            .replace("{name}", sender.avatar.name)
            .replace("{myname}", bot.avatar.name)
