...
class GetToolNode(private val skill: String) : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        val bought =
            org.rsmod.content.other.progressivebots.economy.EconomyManager.buyToolOrWeapon(
                player,
                "obj.$bestTool",
            )
        if (bought) {
        ...
        // If we fail because of GP, spawn the basic level 1 bronze tool for free so we don't get
        // soft-locked
        val fallbackTool = if (skill == "woodcutting") "bronze_axe" else "bronze_pickaxe"
        if ("obj.$fallbackTool" !in player.inv) {
            val coinsId =
                ServerCacheManager.getItem("obj.coins".asRSCM(dev.openrune.rscm.RSCMType.OBJ))?.id
                    ?: 995
            val toolId =
                ServerCacheManager.getItem(
                        "obj.$fallbackTool".asRSCM(dev.openrune.rscm.RSCMType.OBJ)
                    )
                    ?.id ?: return NodeStatus.FAILURE
...
class FindTargetNode(private val skill: String) : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        if (bestCoords != null) {
            ...
            // We use a custom field in GoalStack for locId if needed, or just let
            // InteractTargetNode rescan
            // Actually, we should store locId. Let's just find it again in InteractTargetNode since
            // it's fast
...
class InteractTargetNode : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        val loc =
            locReg.findAll(ZoneKey.from(coords)).firstOrNull { it.coords == coords }
                ?: return NodeStatus.FAILURE
        ...
        fun build(skill: String): BehaviorNode {
            return SelectorNode(
                listOf(
                    ...
                    SequenceNode(listOf(InvertNode(HasToolNode(skill)), GetToolNode(skill))),

                    // 2. If inventory is full, bank the loot
                    SequenceNode(listOf(IsInventoryFullNode(), BankLootNode())),

                    // 3. Otherwise, find a target and gather
                    SequenceNode(listOf(FindTargetNode(skill), InteractTargetNode())),
                )
            )
        }
    }
