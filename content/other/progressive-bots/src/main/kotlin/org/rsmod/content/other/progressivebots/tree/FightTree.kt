...
class HasWeaponNode : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        val level = player.stat("stat.attack")
...
class GetWeaponNode : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        if ("obj.$fallback" !in player.inv) {
            val toolId =
                ServerCacheManager.getItem("obj.$fallback".asRSCM(dev.openrune.rscm.RSCMType.OBJ))
...
class InteractNPCNode : ActionNode() {
    override fun execute(player: Player, state: BotState): NodeStatus {
        ...
        player.routeRequest =
            RouteRequestPathingEntity(destination = npc.avatar, clientRequest = false)
...
class FightTreeBuilder {
    companion object {
        fun build(): BehaviorNode {
            return SelectorNode(
                listOf(
                    SequenceNode(listOf(InvertNode(HasWeaponNode()), GetWeaponNode())),
                    SequenceNode(listOf(FindNPCNode(), InteractNPCNode())),
                )
            )
        }
    }
